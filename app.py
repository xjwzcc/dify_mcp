...
@server.tool()
async def check_room_availability(room_type: str, check_in_date: str) -> str:
    """
    查询酒店特定日期的房态和价格
    参数:
        room_type: 房型(例如: 莳月·宜家, 莳光·景观房)
        check_in_date: 入住日期 (YYYY-MM-DD)
    """
    # 模拟 PMS 数据库查询，您可以后续对接真实的 API
    mock_db = {
...
@server.tool()
async def get_nanjing_weather(date: str) -> str:
    """
    查询南京老门东景区的天气预报，用于行程规划
    参数:
        date: 日期 (例如: 今天, 明天, 2024-05-01)
    """
    # 实际项目中这里应调用高德或和风天气 API
    return "南京老门东明天小雨转阴，气温 15℃-20℃，适合在酒店明清庭院内喝茶，或带把伞漫步老门东青石板路。"

@server.tool()
async def plan_travel_route(destination: str) -> str:
    """
    规划从酒店(老门东)出发到周边景点的交通路线
    参数:
        destination: 目的地名称 (例如: 夫子庙, 南京博物院)
    """
    # 模拟地图 API 逻辑
    routes = {
    ...
    return routes.get(destination, f"已为您查询到前往 {destination} 的路线：建议打车前往，让前台为您呼叫专车。")
